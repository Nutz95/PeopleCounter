...
def build_stream_open_opts(stream_url: str) -> dict[str, str]:
    """Return FFmpeg/PyNvCodec input options tuned for the given stream URL.

    RTSP streams generated by the Windows GStreamer + MediaMTX bridge are
    configured as TCP-only. When FFmpeg/PyNvCodec reconnects with its default
    transport negotiation it may attempt UDP first, which MediaMTX rejects with
    ``461 Unsupported Transport``. That forces the decoder to recreate and then
    wait for the next IDR, causing visible multi-frame drops with long GOPs.

    We therefore force RTSP over TCP here, and keep generous probe settings for
    both HTTP and RTSP so reconnects can re-detect the stream quickly.
    """
    url = (stream_url or "").lower()
...
class NvdecDecoder:
    ...
    def _build_decoder_opts(self) -> dict[str, str]:
        return build_stream_open_opts(self.stream_url)
    ...
    def _probe_stream_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            url_lower = (self.stream_url or "").lower()
            if url_lower.startswith("rtsp://"):
                ffprobe_cmd += ["-rtsp_transport", "tcp"]
            ffprobe_cmd += [
                "-select_streams",
                "v:0",
                "-show_entries",
